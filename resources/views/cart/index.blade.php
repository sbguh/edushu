    ...
    <table class="table table-striped">
      ...
      @foreach($cartItems as $item)
        <tr data-id="{{ $item->book->id }}">
          <td>
            <input type="checkbox" name="select" value="{{ $item->book->id }}" {{ $item->book->on_sale ? 'checked' : 'disabled' }}>
          </td>
          <td class="product_info">
            ...
            <div @if(!$item->book->on_sale) class="not_on_sale" @endif>

              <span class="sku_title">{{ $item->book->name }}</span>
              @if(!$item->book->on_sale)
                <span class="warning">该商品已下架</span>
              @endif
            </div>
          </td>
          <td>
            <input type="text" class="form-control form-control-sm amount" @if(!$item->book->on_sale) disabled @endif name="amount" value="{{ $item->amount }}">
          </td>
          <td>
